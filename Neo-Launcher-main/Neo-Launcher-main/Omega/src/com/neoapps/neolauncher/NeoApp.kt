...
import com.neoapps.neolauncher.util.FirebaseTelemetryConfig
...
//        }
    ...
    private fun configureFirebase() {
        val enableCollection = FirebaseTelemetryConfig.shouldEnableCollection(BuildConfig.BUILD_TYPE)

        runCatching {
            FirebaseAnalytics.getInstance(this).setAnalyticsCollectionEnabled(enableCollection)
            FirebaseCrashlytics.getInstance().setCrashlyticsCollectionEnabled(enableCollection)
        }.onFailure { throwable ->
            Log.w(TAG, "Firebase setup skipped", throwable)
        }
    }
    ...
    private fun configureMobileAds() {
        runCatching {
            ...
            Log.w(TAG, "Mobile Ads setup skipped", throwable)
        }
    }
