...
open class VideoDetectionTabViewModel @Inject constructor(
    ...
    private val sessionRequestedDownloadFileNames = mutableSetOf<String>()
    ...
    override fun onStartPage(url: String, userAgentString: String) {
        if (Looper.myLooper() != Looper.getMainLooper()) {
            mainHandler.post { onStartPage(url, userAgentString) }
            return
        }
        if (url == lastUrl) {
        ...
        if (url != initialUrl) {
            ...
            updateDetectedVideosList(mutableSetOf())
            clearSessionDownloadRequests()
            cancelAllCheckJobs()
        } else {
    ...
    fun onReloadPage(url: String, userAgentString: String) {
        if (Looper.myLooper() != Looper.getMainLooper()) {
            mainHandler.post { onReloadPage(url, userAgentString) }
            return
        }
        lastUrl = url
        downloadButtonState.set(DownloadButtonStateCanNotDownload())
    ...
    open fun startVerifyProcess(
        ...
        updateM3u8LoadingList(loadings?.toMutableSet())
        ...
        verifyVideoLinkJobStorage[taskUrl] =
            io.reactivex.rxjava3.core.Observable.create { emitter ->
                ...
                updateM3u8LoadingList(loadings2?.toMutableSet())
    ...
    open fun pushNewVideoInfoToAll(newInfo: VideoInfo) {
        ...
        if (CopyrightRestrictedSitePolicy.isDownloadRestrictedUrl(newInfo.originalUrl) ||
            CopyrightRestrictedSitePolicy.isDownloadRestrictedUrl(newInfo.firstUrlToString)
        ) {
            setButtonState(DownloadButtonStateCanNotDownload())
            return
        }
        ...
        updateDetectedVideosList(detectedVideos + newInfo)
    ...
    override fun checkRegularVideoOrAudio(
        ...
        if (DirectMediaSupport.shouldIgnoreRegularUrl(clearedUrl)) {
            return null
        }
        ...
        val disposable = io.reactivex.rxjava3.core.Observable.create<Unit> {
            ...
            updateRegularLoadingList(loadings?.toMutableSet())
            propagateCheckJob(uriString, headers, isCheckOnAudio, isCheckOnVideo, isCheckOnImage)
            it.onComplete()
    ...
    override fun cancelAllCheckJobs() {
        updateRegularLoadingList(mutableSetOf())
        updateM3u8LoadingList(mutableSetOf())
    ...
    fun setButtonState(state: DownloadButtonState) {
        if (Looper.myLooper() != Looper.getMainLooper()) {
            mainHandler.post { setButtonState(state) }
            return
        }
        when (state) {
    ...
    private fun updateDetectedVideosList(videos: Set<VideoInfo>) {
        if (Looper.myLooper() == Looper.getMainLooper()) {
            detectedVideosList.set(videos)
        } else {
            mainHandler.post { detectedVideosList.set(videos) }
        }
    }

    fun markSessionDownloadRequested(fileName: String) {
        synchronized(sessionRequestedDownloadFileNames) {
            sessionRequestedDownloadFileNames.add(fileName)
        }
    }

    fun wasRequestedThisSession(fileName: String): Boolean {
        synchronized(sessionRequestedDownloadFileNames) {
            return sessionRequestedDownloadFileNames.contains(fileName)
        }
    }

    private fun clearSessionDownloadRequests() {
        synchronized(sessionRequestedDownloadFileNames) {
            sessionRequestedDownloadFileNames.clear()
        }
    }

    private fun updateRegularLoadingList(loadings: MutableSet<String>?) {
        if (Looper.myLooper() == Looper.getMainLooper()) {
            regularLoadingList.set(loadings)
        } else {
            mainHandler.post { regularLoadingList.set(loadings) }
        }
    }

    private fun updateM3u8LoadingList(loadings: MutableSet<String>?) {
        if (Looper.myLooper() == Looper.getMainLooper()) {
            m3u8LoadingList.set(loadings)
        } else {
            mainHandler.post { m3u8LoadingList.set(loadings) }
        }
    }
    ...
    fun propagateCheckJob(
        ...
        runCatching {
            ...
            okHttpProxyClient.getProxyOkHttpClient().newCall(request).execute().use { response ->
                ...
                val isImage = contentType.contains("image", true)
                ...
                val isImageContent = isImage && isCheckOnImage
        ...
        runCatching {
            ...
            okHttpProxyClient.getProxyOkHttpClient().newCall(request).execute().use { response ->
                ...
                when {
                    ...
                    contentType.contains("image", true) && isCheckOnImage -> {
                        setMediaInfoWrapperFromUrl(
                            finalUrlPairEmpty.first,
                            webTabModel?.getTabTextInput()?.get(),
                            finalUrlPairEmpty.second.toMap(),
    ...
    private fun setMediaInfoWrapperFromUrl(
        ...
        try {
            ...
            val inferredExt =
                DirectMediaSupport.inferExtension(url.toString(), contentType, fallbackExt)
            val inferredTitle = DirectMediaSupport.deriveTitle(
                url.toString(),
                webTabModel?.currentTitle?.get() ?: "no_title"
            )
            ...
            val video = VideoInfoWrapper(
                VideoInfo(
                    ...
                    title = inferredTitle,
                    ext = inferredExt,
                    thumbnail = if (isImage) url.toString() else "",
                        ...
                        mutableListOf(
                            VideoFormatEntity(
                                ...
                                format = when {
                                    isAudio -> "audio"
                                    isImage -> "image"
                                    else -> ContextUtils.getApplicationContext()
