...
open class VideoServiceLocal(
    ...
    private fun handleYoutubeDlUrl(
        ...
        try {
            ...
            return VideoInfoWrapper(
                VideoInfo(
                    title = info.title?.takeIf { it.isNotBlank() } ?: fallbackTitle(url.url.toString()),
                    formats = listFormats
                ).apply {
    ...
    private fun fallbackTitle(rawUrl: String): String {
        val uri = Uri.parse(rawUrl)
        val lastSegment = uri.lastPathSegment?.substringBefore('?')?.takeIf { it.isNotBlank() }
        return lastSegment ?: uri.host.orEmpty()
    }
}
