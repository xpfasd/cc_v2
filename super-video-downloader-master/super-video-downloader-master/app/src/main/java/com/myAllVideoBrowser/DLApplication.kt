...
import com.myAllVideoBrowser.util.proxy_utils.ProxyRoutingMode
...
open class DLApplication : DaggerApplication() {
    ...
    fun startProxyService() {
        ...
        if (!ProxyFeatureGate.isLocalProxyAvailable()) {
            AppLogger.i("Proxy service startup skipped because proxy support is disabled.")
            return
        }

        val shouldEnableProxy = ProxyRoutingMode.shouldEnableLocalProxy(
            isProxyEnabled = sharedPrefHelper.getIsProxyOn(),
            isDohEnabled = sharedPrefHelper.getIsDohOn()
        )
        if (!shouldEnableProxy) {
            AppLogger.d("Proxy service skipped on startup because direct connection is active.")
            return
        }
