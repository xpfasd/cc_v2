...
class ProxiesViewModel @Inject constructor(
    ...
    val isSecureDnsOn = ObservableField(false)
    ...
    override fun start() {
        viewModelScope.launch(Dispatchers.IO) {
            val isProxyAvailable = ProxyFeatureGate.isLocalProxyAvailable()
            isSecureDnsOn.set(isProxyAvailable && sharedPrefHelper.getIsDohOn())
            ...
            isProxyOn.set(isProxyAvailable && sharedPrefHelper.getIsProxyOn())
    ...
    fun setSecureDnsState(isOn: Boolean) {
        viewModelScope.launch(Dispatchers.IO) {
            val nextState = ProxyFeatureGate.isLocalProxyAvailable() && isOn
            isSecureDnsOn.set(nextState)
            sharedPrefHelper.setIsDohOn(nextState)
    ...
    fun turnOnProxy() {
        viewModelScope.launch(Dispatchers.IO) {
            val nextState = ProxyFeatureGate.isLocalProxyAvailable()
            sharedPrefHelper.setIsProxyOn(nextState)
            proxyController.updateProxyState()
            isProxyOn.set(nextState)
