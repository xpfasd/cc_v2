...
import com.myAllVideoBrowser.util.DownloadStoragePermissionPolicy
...
class ProgressFragment : BaseFragment() {
    ...
    private var pendingDownloadInfo: VideoInfo? = null

    private val storagePermissionLauncher =
        registerForActivityResult(ActivityResultContracts.RequestMultiplePermissions()) { result ->
            val pendingInfo = pendingDownloadInfo
            pendingDownloadInfo = null
            if (pendingInfo != null && result.values.all { it }) {
                completeDownloadStart(pendingInfo)
            }
        }
    ...
    private fun onParsedVideoDownload(videoInfo: VideoInfo, videoTitle: String, format: String) {
        ...
        startDownloadOrRequestPermissions(info)
    }

    private fun startDownloadOrRequestPermissions(info: VideoInfo) {
        if (shouldRequestDownloadPermissions() && !hasDownloadPermissions()) {
            pendingDownloadInfo = info
            storagePermissionLauncher.launch(requiredDownloadPermissions())
            return
        }
        completeDownloadStart(info)
    }
    ...
    private fun shouldRequestDownloadPermissions(): Boolean {
        return DownloadStoragePermissionPolicy.requiresPermissionBeforeDownload(
            isExternalStorageUse = FileUtil.IS_EXTERNAL_STORAGE_USE,
            isAppDataDirUse = FileUtil.IS_APP_DATA_DIR_USE
        )
    }

    private fun hasDownloadPermissions(): Boolean {
        return requiredDownloadPermissions().all { permission ->
    ...
    private fun requiredDownloadPermissions(): Array<String> {
        return DownloadStoragePermissionPolicy.requiredPermissions(Build.VERSION.SDK_INT)
    }
