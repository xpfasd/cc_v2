...
class CandidatesListRecyclerViewAdapter(
    ...
    private companion object {
        private const val INVALID_FORMAT = "__invalid_format__"
    }
    ...
    override fun onBindViewHolder(holder: CandidatesViewHolder, position: Int) {
        ...
        val candidate = formatEntity.format ?: INVALID_FORMAT

        with(holder.binding) {
            val selected = selectedFormat.get()?.get(downloadCandidates.id)
            val isSelected = candidate == selected

            this.cardItem.setCardBackgroundColor(
                ContextCompat.getColor(
                    this.root.context,
                    if (isSelected) R.color.detected_chip_selected else R.color.detected_chip_surface
                )
            )
            this.cardItem.strokeColor = ContextCompat.getColor(
                this.root.context,
                if (isSelected) R.color.detected_chip_selected_stroke else R.color.detected_chip_stroke
            )

            this.videoInfo = downloadCandidates
            this.downloadCandidate = candidate
            this.isCandidateSelected = isSelected
            this.tvTitle.text = getShortOfFormat(
                candidate,
                downloadCandidates.isDetectedBySuperX,
                holder.itemView.context.getString(R.string.common_error_label)
            )
            ...
            this.tvTitle.setTextColor(
                ContextCompat.getColor(this.root.context, R.color.processing_text_primary)
            )
            this.tvData.setTextColor(
                ContextCompat.getColor(this.root.context, R.color.detected_text_secondary)
            )
    ...
    private fun getShortOfFormat(
        format: String?,
        detectedBySuperX: Boolean,
        errorLabel: String
    ): String {
        val formattedFormat =
            makeVideoFormatHumanReadable(format ?: INVALID_FORMAT, detectedBySuperX)
        if (formattedFormat != INVALID_FORMAT) {
