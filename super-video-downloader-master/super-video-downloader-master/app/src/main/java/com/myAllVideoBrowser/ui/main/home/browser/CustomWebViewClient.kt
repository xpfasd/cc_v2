...
class CustomWebViewClient(
    ...
    override fun shouldInterceptRequest(
        ...
        if (DirectMediaSupport.shouldIgnoreRegularUrl(url) && !url.contains(".m3u8") && !url.contains(".mpd")) {
            return super.shouldInterceptRequest(view, request)
        }
        ...
        if (isCheckOnMp4 || isCheckM3u8 || isCheckOnAudio || isCheckOnImage) {
            ...
            when {
                ...
                contentType == ContentType.M3U8 || contentType == ContentType.MPD || url.contains(".m3u8") || url.contains(
                    ...
                    if ((isCheckOnMp4 || isCheckOnAudio || isCheckOnImage) && contentType != ContentType.OTHER) {
    ...
    override fun onPageStarted(view: WebView, url: String, favicon: Bitmap?) {
        ...
        updateTabEvent.value = WebTab(
            ...
            title = view.title,
            iconBytes = favi,
            previewBytes = pageTab.getPreviewBytes(),
            headers = headers,
            webview = view,
            id = pageTab.id
        )
    ...
    override fun onPageFinished(view: WebView, url: String) {
        ...
        updateTabEvent.value = WebTab(
            url = url,
            title = view.title ?: pageTab.getTitle(),
            iconBytes = view.favicon ?: pageTab.getFavicon(),
            previewBytes = WebTabPreviewCapture.capture(view) ?: pageTab.getPreviewBytes(),
            headers = pageTab.getHeaders() ?: emptyMap(),
            webview = view,
            id = pageTab.id
        )
    }
